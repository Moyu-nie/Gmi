...
Gmi <- function(X, y, beta, penalty = "lasso",
  ...
  for (k in 1: n.lambda) {
    ...
    for(ite in 1:max.iter){
      if(ite == 1){
        cd.temp1 = SBfusedlasso(X = X, y = y, a_0 = a_0, beta = beta, lam = lam,
      ...
      if((ite > 1) & (length(ind1) >= 2)){
        cd.temp2 = SBfusedlasso(X = X[,ind1], y = y, a_0 = a_0, beta = beta[ind1], lam = lam, eta = eta, rho1 = 1, rho2 = 1, penalty.type =  pentype ,
        ...
        cd.temp3 = SBfusedlasso(X = X, y = y, a_0 = a_0, beta = beta2, lam = lam, eta = eta, rho1 = 1, rho2 = 1, penalty.type =  pentype ,
                                          epsilon1 = 1e-7, epsilon2 = 1e-7, maxiter1 = 100, maxiter2 = 20, pf = pf)
        a_0 = cd.temp3$a_0
        beta = cd.temp3$beta
  ...
  val = list(beta = beta, a_0 = a_0list[cri.loc], a_0.list = a_0list, beta.m.mat = beta.mat, beta.i.mat = beta.list.inter,beta.m = beta.mat[ind.list.main[[cri.loc]], cri.loc], beta.i = beta.i,lambda = lambda[1:k], mainInd.list = ind.list.main[1:k],
